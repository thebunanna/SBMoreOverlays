package thebunanna.SBmoreoverlays.itemsearch;
...
import thebunanna.SBmoreoverlays.Proxy;
import thebunanna.SBmoreoverlays.config.Config;
import thebunanna.SBmoreoverlays.utils.GuiUtil;
...
public class GuiHandler {
    ...
    private static String text = StatCollector.translateToLocal("gui."+ SBMoreOverlays.MOD_ID+".search.disabled");
    ...
    public void onGuiInit(GuiScreenEvent.InitGuiEvent.Post event) {
        ...
        try {
            ...
            SBMoreOverlays.logger.error("Something went wrong. Tried to load gui coords with java reflection. Gui class: "+gui.getClass().getName());
            e.printStackTrace();
    ...
    private static boolean isSearchedItem(ItemStack stack){
        ...
        String fil = JeiModule.filter.getFilterText().toLowerCase();
...
//        	}
//        	
//        	System.out.println(nbt.getCompoundTag("display").toString());
        	sbName = nbt.getCompoundTag("display").getString("Name").toLowerCase();
        	//lore = String.join(" ",nbt.getCompoundTag("display").getCompoundTag("lore").toString().toLowerCase().split("§."));
        	
        	NBTTagList ntag = nbt.getCompoundTag("display").getTagList("Lore", Constants.NBT.TAG_STRING);
        	for (int i = 0; i < ntag.tagCount(); i++) {
        	//	System.out.println(ntag.getStringTagAt(i));
        		String nt = ntag.getStringTagAt(i).toLowerCase();
        		if (nt.length() < 20) {
        			lore.add(nt);
        		}
        	}
        ...
        for(ItemStack stack1 : itemCache){
            if (stack1.isItemEqual(stack) 
            		|| (stack1.getItem() == stack.getItem() && stack1.getItem().isDamageable()))
                return true;
            
        }
        //if (stack.getUnlocalizedName().contains("d")) return true;
        if (sbName.contains(fil)) {
        	return true;
        }
        
        if (fil.length() > 4) {        	
        	//System.out.println(lore);
        	if (fil.substring(0, 4).equals("[en]")) {
        		String filtered = fil.substring(4);
        		for (String x : lore) {
        			if (x.contains(filtered)) {
        				return true;
        			}
        		}
        		return false;
    ...
    public static void toggleMode(){
        ...
        if(enabled){
            ...
            JeiModule.filter.getItemList().forEach((itemElement) -> itemCache.add(itemElement.getItemStack()));
            emptyFilter = lastFilterText.replace(" ","").isEmpty();
            text = StatCollector.translateToLocal("gui."+ SBMoreOverlays.MOD_ID+".search.enabled");
        }
        else {
            lastFilterText = "";
            if(itemCache!=null)
                itemCache.clear();
            text = StatCollector.translateToLocal("gui."+ SBMoreOverlays.MOD_ID+".search.disabled");
        }
